...
export default function SearchUsuarios() {
  ...
  const fetchMoreData = useCallback(async () => {
    ...
    setIsLoading(true);
    const newData = await pageRequestHandler(
  ...
  return (
    <>
      <div id="root-about" className="h-max w-full overflow-y-auto bg-black">
        <div className="p-8">
          <h1 className="mb-4">
            Resultados da busca: "{searchQuery}" ({results.page.totalElements}{" "}
            resultados)
          </h1>
          <ListView scrollSentinelRef={scrollSentinelRef}>
            {results.content.map((x) => (
              <UsuarioItem key={x.uuid} usuario={x} />
            ))}
          </ListView>
        </div>
      </div>
